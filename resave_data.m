...
for img_num = 1:length(img_fnames)
    ...
    clas_inpained = imresize(clas_inpained,2,"nearest");
    clas_inpained(mask == 0) = 0;

    rgb = zeros(size(clas_inpained,1),size(clas_inpained,2),3);

    rgb(:,:,2) = clas_inpained == 50;
    rgb(:,:,1) = clas_inpained == 100;
    rgb(:,:,3) = clas_inpained == 150;
    rgb = uint8(rgb * 255);
